...
export default function AssetDetails() {
  ...
  useEffect(() => {
    Promise.all([getAssetById(id), getAllocationHistoryByAsset(id)])
      .then(([aRes, hRes]) => {
        setAsset(aRes.data);
  ...
  const handleReport = async () => {
    ...
    setActionError(null);
    try {
      // POST /api/reports  { assetId, issueTitle, description }
  ...
  const isExpired =
    ...
    new Date(asset.warrantyExpirationDate) < new Date();

  const isExpiringSoon =
    !isExpired &&
    asset.warrantyExpirationDate &&
    new Date(asset.warrantyExpirationDate) - Date.now() < 30 * 24 * 60 * 60 * 1000;
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      <div className="card" style={{ marginBottom: "1.25rem", position: "relative", overflow: "hidden" }}>
        ...
        {(isExpired || isExpiringSoon) && (
          ...
          >
            ⚠{" "}
            {isExpired
              ? "Warranty expired — consider reassigning as spare or decommissioning."
      ...
      <div className="card">
        ...
        {history.length === 0 ? (
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>No allocation history yet.</p>
        ) : (
              ...
              <div key={h.id || i} style={{ display: "flex", gap: "0.9rem", paddingBottom: "1rem", position: "relative" }}>
                ...
                <div>
                  ...
                  <p style={{ margin: "2px 0 0", fontSize: "0.8rem", color: "var(--text-muted)" }}>
                    ...
                    {h.returnedAt
                      ? ` → ${new Date(h.returnedAt).toLocaleDateString()}`
                      : " → Present"}
                  </p>
                </div>
              </div>
      ...
      {assignModal && (
        <Modal
          title={asset.assignedTo ? "Reassign Asset" : "Assign Asset"}
          onClose={() => { setAssignModal(false); setActionError(null); }}
        >
        ...
        <Modal title="Report an Issue" onClose={() => { setIssueModal(false); setActionError(null); }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            <input
              ...
              placeholder="Issue title (e.g. Battery draining fast)"
              style={inputStyle}
            />
            <textarea
              value={issueDesc}
...
function Detail({ label, value, color }) {
  return (
    <div>
      <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 0.8 }}>
        {label}
      </p>
      <p style={{ margin: "3px 0 0", fontSize: "0.9rem", fontWeight: 600, color: color || "var(--text-main)" }}>
        {value}
      </p>
    </div>
  );
...
function Modal({ title, onClose, children }) {
  return (
    ...
    >
      <div
        className="card"
        style={{ width: "100%", maxWidth: 440, animation: "fadeIn 0.2s ease" }}
      >
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.6rem", marginTop: "1.25rem" }}>
      ...
      <button
        ...
        className="btn"
        style={{
          background: confirmColor || "var(--primary)",
          color: "white",
