...
const INITIAL = {
  ...
  serialNumber:       "",
  purchaseDate:       "",
  warrantyExpiration: "",   // UI field; mapped to warrantyExpirationDate on submit
};
...
export default function AddAsset({ editMode = false }) {
  ...
  // In edit mode, pre-fill the form
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode && id) {
        // PUT /api/assets/{id}  — only updatable fields
    ...
    <div className="animate-fade-in" style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          {/* Type selector — disabled in edit mode (can't change type) */}
          {!editMode && (
            <div>
          ...
          {/* Serial number — readonly in edit mode */}
          {!editMode && (
            <Field
              label="Serial Number"
